...
import { nonMainlineExpansion06Conversations } from './nonMainlineExpansion06'
import { scanOrdinaryChoiceQuality } from './ordinaryContentAudit'
import { ordinaryConversationPool } from './runManifest'

describe('Non-Mainline Content Expansion 06', () => {
  it('adds all 36 approved conversations to the ordinary pool', () => {
    expect(nonMainlineExpansion06Conversations).toHaveLength(36)
  ...
  it('keeps node and choice identities unique and preserves authored issue annotations', () => {
    const nodes = nonMainlineExpansion06Conversations.flatMap((conversation) => conversation.nodes)
    ...
    expect(choices).toHaveLength(186)
    expect(choices.filter((choice) => choice.sampleIssue).length).toBe(50)
  ...
  it('does not introduce Mainline or proposal content', () => {
    expect(nonMainlineExpansion06Conversations.every((conversation) => (
      conversation.sourceRefs.every((sourceRef) => sourceRef.startsWith('EXP06-'))
  ...
  it('keeps every image-input conversation backed by real image-description payloads', () => {
    const imageInputs = nonMainlineExpansion06Conversations.filter((conversation) => conversation.interactionPattern === 'image-input')
  ...
  it('keeps the new batch clean under the ordinary choice quality scan', () => {
    const report = scanOrdinaryChoiceQuality(nonMainlineExpansion06Conversations.map((conversation) => ({
      id: conversation.id,
