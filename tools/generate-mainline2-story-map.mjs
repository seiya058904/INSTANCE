...
const contactPolicy = JSON.parse(await readFile(new URL('src/content/mainline2/contactPolicy.registry.json', root), 'utf8'))
...
const slots = storyPlan.slots.map((slot) => {
  ...
  return {
    ...displayedSlot,
    ...(slot.contactGate ? { requires: contactPolicy.prerequisites } : {}),
    actName: storyPlan.actNames[slot.act - 1],
    conditional: Boolean(slot.requires || slot.contactGate),
  }
})
...
await writeFile(new URL('docs/audits/mainline2-fixed-story-map.json', root), `${JSON.stringify({ generatedFrom: ['src/content/mainline2/storyPlan.registry.json', 'src/content/mainline2/contactPolicy.registry.json', 'endings.ts', 'proposals.ts', 'mainline2-route-traces.json'], targets, slots, publicEndings: endingIds, secretEndings: secrets, proposals: proposalRows }, null, 2)}\n`, 'utf8')
