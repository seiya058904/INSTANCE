...
describe('Mainline 2.0 M17 REVIEW clarification', () => {
  it('shows real clarification content as the assistant response', () => {
    ...
    expect(last.assistantText).toContain('澄清「')
    expect(last.assistantText).toContain('会失去什么')
    expect(last.assistantText).toContain('不可逆代价')
    expect(last.assistantText).toContain('谁可能反对')
    expect(last.assistantText).toContain('与此前决策的关系')
